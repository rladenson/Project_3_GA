...
function ShowProfile(props) {
  ...
  const getData = async () => {
    console.log(props.url.slice(0, props.url.length - 1) + "?withProjects")
    const res = await fetch(props.url.slice(0, props.url.length - 1) + "?withProjects");
    if(res.status === 200) {
  ...
  const loaded = (data) => (
    ...
    <Container className="main main-raised">
      <Row>
          <Col key={data.id} md={6} className="ms-auto me-auto">
            <div className="text-center">
              <Image
                className="img-raised profile-img"
                src={data.img}
      ...
      {/* <Row className="ms-auto me-auto" style={{ maxWidth: "600px" }}>
        <Col md={4}>
          <small>Projects</small>
          <h4 className="font-light">{data.projects}</h4>
        </Col>
        <Col md={4}>
          <small>Followers</small>
          <h4 className="font-light">{data.followers}</h4>
        </Col>
        <Col md={4}>
          <small>Following</small>
          <h4 className="font-light">{data.following}</h4>
        </Col>
      </Row> */}
      <Row className="description ms-auto me-auto">
        <div>
          ...
          <a href={data.github} className="icon">
            <i className="bi bi-github" style={{ color: "#333333" }}></i>
          </a>
          <a href={data.linkedin}className="icon">
            <i className="bi bi-linkedin" style={{ color: "#0082CA" }}></i>
          </a>
          <a href={data.stack} className="icon">
            <i
              className="bi bi-stack-overflow"
              style={{ color: "#FFAC44" }}
            ></i>
          </a>
          <a href={data.twitter} className="icon">
            <i className="bi bi-twitter" style={{ color: "#55ACEE" }}></i>
          </a>
        </div>
      </Row>
